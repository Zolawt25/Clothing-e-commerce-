...
const Navbar = ({carts}) => {
    ...
    return (
        <div className={`${navbar && "shadow-lg"} flex justify-center bg-navbg sticky top-0 z-50`}>
                <div className={`flex justify-between items-center px-10 py-3 sm:px-16 md:px-24 md:py-4  bg-navbg z-50 relative w-full max-w-[1400px]`}>
            <div><a href="/"><img src="/img/logo.png" alt="logo" /></a></div>
            <div className='flex items-center'>
                <div className={`relative ${!isOpen && "overflow-hidden"} md:overflow-auto`}>
                    <div className={`flex items-center flex-col border-l duration-300 absolute md:top-0 top-[-30px] ${isOpen ? "right-[-180px]" : "right-[-1000px]"} bg-navbg h-screen w-[90vw] sm:w-[60vw] justify-center text-xl gap-10 md:relative md:flex-row md:bg-transparent md:w-auto md:h-auto z-30 md:text-[15px] md:justify-normal md:gap-0 md:right-0`}>
